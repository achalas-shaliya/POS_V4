...
const NAV_ITEMS = [
  ...
  { href: "/dashboard/sales-history", label: "Sales History", permission: "sales:read" },
  ...
  { href: "/dashboard/returns", label: "Returns", permission: "returns:read" },
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col md:grid md:h-screen md:grid-cols-[250px_1fr] md:gap-4 md:overflow-hidden md:px-6 md:py-4">
      {/* ── Mobile backdrop ── */}
      {sidebarOpen && (
        <div
      ...
      {/* ── Sidebar ── */}
      <aside
        className={`fixed inset-y-0 left-0 z-30 flex w-72 flex-col rounded-r-2xl border-r border-line bg-surface-elevated p-5 shadow-xl transition-transform duration-200 md:static md:flex md:h-full md:w-auto md:translate-x-0 md:overflow-y-auto md:rounded-2xl md:border md:shadow-sm md:p-5 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {sidebarContent}
      </aside>
